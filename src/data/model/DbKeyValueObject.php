...
class DbKeyValueObject implements \IteratorAggregate
{
	protected 
      $_connection = null,
      $_table = null,
      $_keyField = 'attribute_key',
      $_valueField = 'attribute_value',
      $_typeField = 'attribute_type',
      $_keyValues = array(),
      $_modifiedKeys = array(),
      $_filters = array(),
      $_loaded = false;
    ...
    * @param oxide\data\Connection $conn[optional]
...
//         throw new \Exception('filters are not defined');
    ...
    */
	public function deleteAll(data\Connection $db = null)
	{
		...
		require_once 'Oxide/Database/DbDeleteQuery.php';
		$query = new DbDeleteQuery($this->_db, $this->_table);
		foreach($this->_filters as $key => $value) {
			$query->where($key, $value);
		}
...
//			throw new \Exception('Filters are not set ' . __METHOD__);
    ...
    */
	protected function _update($key)
	{
		...
		$row = array($this->_valueField => $this->_keyValues[$key], $this->_keyField => $key);
		$query->execute($row);
	}
    ...
    *
    * @access public
    * @return \oxide\data\model\ArrayIterator 
    */
