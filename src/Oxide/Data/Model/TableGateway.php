...
use Oxide\Data\Sql\DeleteQuery;
use Oxide\Data\Sql\InsertQuery;
use Oxide\Data\Sql\Query;
use Oxide\Data\Sql\SelectQuery;
use Oxide\Data\Sql\UpdateQuery;
...
class TableGateway
{
    ...
    public function discoverSchema()
    {
        ...
        $query = new SelectQuery('INFORMATION_SCHEMA.COLUMNS', $conn);
        $query->select([
            'column_default',
            'column_name',
            'data_type',
    ...
    {
        if (!$this->onPreInsert($obj)) {
            return false;
        }

        $query = new InsertQuery($this->getTableName(), $this->getConnection());
        // filter data
    ...
    public function update(DataObject $obj)
    {
        ...
        $query = new UpdateQuery($this->getTableName(), $this->getConnection());
    ...
    {
        if (!$this->onPreDelete($obj)) {
            return;
        }

        $query = new DeleteQuery($this->getTableName(), $this->getConnection());
        $this->addPrimaryKeysToQuery($query, $obj);
        $count = $query->execute();
     ...
     * @throws \Exception
     */
    protected function addPrimaryKeysToQuery(Query $query, DataObject $obj)
    {
    ...
    protected function onSelect(SelectQuery $query)
    {

    }

    protected function onPreInsert(DataObject $obj)
    {
        return true;
    }
    ...
    protected function onPreUpdate(DataObject $obj)
    {
        return true;
    }
    ...
    protected function onPreDelete(DataObject $obj)
    {
        return true;
    }
