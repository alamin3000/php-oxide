...
class ViewManager
{
    ...
    public function resolveTemplateScript($script)
    {
        ...
        // add directories to look for, in order
        $dirs = [
            $this->getTemplateDir() . "/{$module}/{$controller}",         // template directory
            $moduleDir . '/' . $this->_viewDirName . "/{$controller}"   // module directory
        ];

        // check if file extension is given, if not add it
        ...
        // attempt to find script withing given order
        foreach ($dirs as $dir) {
            $file = "{$dir}/{$scriptfile}";
            if (file_exists($file)) {
