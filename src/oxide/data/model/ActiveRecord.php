...
abstract class ActiveRecord extends DataObject
{
     ...
     * @return Connection
     */
    public static function sharedConnection(Connection $conn = null)
    {
        if ($conn) {
    ...
    public function delete()
    {
        ...
        $query = new DeleteQuery(static::getTable(), $conn);
    ...
    /**
     * @return bool|\Oxide\Data\Sql\type
     * @throws Exception
     */
    ...
    protected function _update()
    {
        ...
        $query = new UpdateQuery($table, $db);
        $query->where($pkfield, $pkvalue);
        $query->set($data);
        $result = $query->execute();
     ...
     * @return Statement
     */
    public static function select(SelectQuery &$query = null)
    {
        $table = static::getTable();
        if ($query == null) {
        ...
        if ($fields) {
            ...
            foreach ($fields as $field) {
                if (!array_key_exists($i, $arg)) {
                    throw new Exception("$method is missing value");
                }

                if (is_null($arg[$i])) {
                    $query->where($field, null, Connection::OP_NULL);
                } else {
    ...
    // hock functitons
    protected static function onPreSelect(SelectQuery $select)
    {
    }

    protected static function onPostSelect(SelectQuery $stmt)
    {
    }
