...
class EAVModel implements IteratorAggregate
{
     ...
     * @param array $filters
     */
    public function __construct(Connection $connection = null, $table = null, $filters = array())
    {
        if ($connection) {
    ...
    {
        if (isset($param['key'])) {
            $this->_keyField = $param['key'];
        }
        if (isset($param['value'])) {
            $this->_valueField = $param['value'];
        }
    }
    ...
    public function connection(Connection $conn = null)
    {
        ...
        if ($this->_connection === null) {
            $this->_connection = Connection::sharedInstance();
        }
     ...
     * @return int
     */
    public function deleteAll(Connection $db = null)
    {
        if ($db) {
            $this->_db = $db;
        }
        ...
        $query = new DeleteQuery($this->_db, $this->_table);
        foreach ($this->_filters as $key => $value) {
            $query->where($key, $value);
        }
