...
trait ArrayObjectExtendTrait
{
    ...
    public function insert($value, $offset = null, $index = null)
    {
        ...
        if ($index === null) {
            ...
            if ($offset) {
                ...
                $array = array_slice($array, 0, $index, true) + $arrvalue + array_slice($array, $index, null, true);
            } else {
                $value = [
                    $value
                ];
    ...
    /**
     * Merge contents from given $arr
     *
     * @param array $arr
     */
    public function merge(array $arr)
    {
        $this->exchangeArray(array_replace_recursive($this->getArrayCopy(), $arr));
    }
